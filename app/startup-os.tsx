...
type LiveAgentResult={score:string;title:string;summary:string;items:string[];mode:"live"|"demo";runId:string};
...
function AnalysisResult({result,onApprove,approved}:{result:LiveAgentResult;onApprove:()=>void;approved:boolean}){
  return <section className="analysis-result"><div className="analysis-score"><b>{result.score}</b><small>{result.mode==="live"?"live AI":"demo fallback"}</small></div><div><span className="eyebrow mint">ANALYSIS COMPLETE · {result.mode.toUpperCase()}</span><h2>{result.title}</h2><p>{result.summary}</p><ol>{result.items.map(i=><li key={i}>{i}</li>)}</ol></div><button onClick={onApprove} disabled={approved}>{approved?"✓ Approved":"Approve action"}</button></section>;
}
