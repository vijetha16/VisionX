...
export const posts = sqliteTable("posts", { id:text("id").primaryKey(), authorId:text("author_id").notNull(), organizationId:text("organization_id").notNull(), content:text("content").notNull(), createdAt:text("created_at").notNull() });
export const postLikes = sqliteTable("post_likes", { postId:text("post_id").notNull(), userId:text("user_id").notNull(), createdAt:text("created_at").notNull() });
export const postComments = sqliteTable("post_comments", { id:text("id").primaryKey(), postId:text("post_id").notNull(), authorId:text("author_id").notNull(), content:text("content").notNull(), createdAt:text("created_at").notNull() });
export const teamInvites = sqliteTable("team_invites", { id:text("id").primaryKey(), organizationId:text("organization_id").notNull(), inviterId:text("inviter_id").notNull(), email:text("email").notNull(), role:text("role").notNull(), status:text("status").notNull(), createdAt:text("created_at").notNull() });
export const agentRuns = sqliteTable("agent_runs", { id:text("id").primaryKey(), organizationEmail:text("organization_email").notNull(), agentType:text("agent_type").notNull(), input:text("input").notNull(), status:text("status").notNull(), outputJson:text("output_json").notNull(), mode:text("mode").notNull(), createdAt:text("created_at").notNull(), completedAt:text("completed_at") });
export const agentAuditLog = sqliteTable("agent_audit_log", { id:text("id").primaryKey(), runId:text("run_id").notNull(), actorEmail:text("actor_email").notNull(), event:text("event").notNull(), detail:text("detail").notNull(), createdAt:text("created_at").notNull() });
export const customizationRules = sqliteTable("customization_rules", { id:text("id").primaryKey(), organizationId:text("organization_id").notNull(), authorId:text("author_id").notNull(), title:text("title").notNull(), requirement:text("requirement").notNull(), targetModule:text("target_module").notNull(), level:integer("level").notNull(), compatibility:integer("compatibility").notNull(), status:text("status").notNull(), planJson:text("plan_json").notNull(), version:integer("version").notNull(), createdAt:text("created_at").notNull(), updatedAt:text("updated_at").notNull() });
export const customizationAudit = sqliteTable("customization_audit", { id:text("id").primaryKey(), ruleId:text("rule_id").notNull(), organizationId:text("organization_id").notNull(), actorId:text("actor_id").notNull(), event:text("event").notNull(), snapshotJson:text("snapshot_json").notNull(), createdAt:text("created_at").notNull() });
